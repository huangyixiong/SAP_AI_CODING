...
cd ..
```
> `npm run dev` 会在启动前自动执行一次 MCP build，但首次仍建议手动执行一次，便于定位依赖问题。
...
这将先构建 MCP，再并行启动后端和前端服务。后端会通过 `MCP_SERVER_PATH` 使用已构建的 MCP 入口。

> 💡 **提示**: 请先执行 `npm run install:all` 安装依赖；`npm run dev` 不会自动安装依赖。
